...
test('store creates a new property with valid data', function () {
    ...
    // Create a mock Request that will pass validation
    $request = Mockery::mock(Request::class);
    $request->shouldReceive('all')
        ->andReturn($propertyData);

    // By default, Laravel's validation will check if user_id exists
    // For testing, we need to mock this validation to pass
    ...
    expect($response->getStatusCode())->toBe(201)
        ->and($responseData)->toHaveKey('success', true)
        ->and($responseData)->toHaveKey('data')
        ->and($responseData['data'])->toHaveKey('name', 'New Property');
});
...
test('show returns property details for valid ID', function () {
    ...
    expect($response->getStatusCode())->toBe(200)
        ->and($responseData)->toHaveKey('success', true)
        ->and($responseData)->toHaveKey('data')
        ->and($responseData['data'])->toHaveKey('id', 1);
});
...
test('update modifies property with valid data', function () {
    ...
    expect($response->getStatusCode())->toBe(200)
        ->and($responseData)->toHaveKey('success', true)
        ->and($responseData)->toHaveKey('data')
        ->and($responseData['data'])->toHaveKey('name', 'Updated Property');
});
